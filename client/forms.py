...
class OrderForm(forms.Form):
    order_type = forms.ChoiceField(required=True, choices=[('Buy', 'Buy'), ('Sell', 'Sell')], widget=forms.RadioSelect)
    quantity = forms.IntegerField(required=True, widget=forms.NumberInput(attrs={'placeholder': 'Required'}))
    portfolio = forms.ModelChoiceField(queryset=models.Portfolio.objects.none(), widget=autocomplete.ModelSelect2(url='portfolio-autocomplete'))
    stock = forms.ModelChoiceField(queryset=models.Stock.objects.none(), widget=autocomplete.ModelSelect2(url='stock-autocomplete', forward=['order_type', 'portfolio', 'quantity']))
    exchange = forms.ModelChoiceField(queryset=models.Exchange.objects.none(), widget=autocomplete.ModelSelect2(url='exchange-autocomplete', forward=['stock']))
    broker = forms.ModelChoiceField(queryset=models.Broker.objects.none(), widget=autocomplete.ModelSelect2(url='broker-autocomplete', forward=['exchange']))
    price = forms.FloatField(required=True, widget=forms.NumberInput(attrs={'placeholder': 'Required'}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Div('portfolio', css_class='input-xlarge'), 
